...
class ResNet_Cifar(nn.Module):
    ...
    def __init__(self, block, layers,cfg=None, num_classes=10):
        ...
        cfg1=cfg[0:layers[0]]
        cfg2=cfg[layers[0]:layers[0]+layers[1]]
        cfg3=cfg[layers[0]+layers[1]:layers[0]+layers[1]+layers[2]]
        self.inplanes = 16
        self.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(16)
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, 16, layers[0],cfg=cfg1)
        self.layer2 = self._make_layer(block, 32, layers[1], stride=2,cfg=cfg2)
        self.layer3 = self._make_layer(block, 64, layers[2], stride=2,cfg=cfg3)
    ...
    def _make_layer(self, block, planes, blocks, stride=1,cfg=None):
        ...
        layers = []
        #print(self.inplanes)
        layers.append(block(self.inplanes, planes, stride, downsample,cfg=cfg[i]))
        i =i+1
...
def resnet20_cifar(cfg=None,**kwargs):
    print("20")
    model = ResNet_Cifar(BasicBlock, [3, 3, 3], cfg=cfg,**kwargs)
    return model


def resnet32_cifar(cfg=None,**kwargs):
    print('32')
    model = ResNet_Cifar(BasicBlock, [5, 5, 5], cfg=cfg, **kwargs)
    return model


def resnet44_cifar(cfg=None,**kwargs):
    model = ResNet_Cifar(BasicBlock, [7, 7, 7], cfg=cfg, **kwargs)
    return model


def resnet56_cifar(cfg=None,**kwargs):
    print("56")
    model = ResNet_Cifar(BasicBlock, [9, 9, 9], cfg=cfg,**kwargs)
    return model
